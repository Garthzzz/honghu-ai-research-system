...
- [x] [本阶段必须] 修复 VM legacy health、候选进程与 evidence 生命周期兼容：8080 的 HTTP 可达性、可用身份字段和字段缺失分开记录并使用有界稳定采样；CIM/Get-Process 可选属性不再由 StrictMode 误判；stale record 只有在双进程源、端口和健康探针共同证明候选不存在时才归档收口；原始 gate state/comparison/reasons、主失败、cleanup/pointer recovery 和 post-cleanup/final-state 分区持久化，后续采样不得覆盖原 gate。
