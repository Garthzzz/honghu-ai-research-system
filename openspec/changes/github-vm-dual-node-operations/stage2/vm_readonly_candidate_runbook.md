...
`pre_state.production_sampling` 和 `gate.post_state.production_sampling` 各保留三次有界采样的完整结果。至少两个可用样本必须形成唯一的硬权威身份 cluster，且全部可用样本必须属于同一 cluster；证据会列出 `selected_quorum_attempts`、冲突 attempts、authority clusters，以及逐样本 listener PID。孤立的 HTTP/解析失败可以作为 warning 保留，但任何可用样本的 hard-authority 冲突、可用样本不足、pre/post 的 release/app/manifest/current/broadcast 身份变化、8080 listener 消失，或候选 PID 出现在 8080 上仍严格失败。listener PID set 的瞬时漂移会如实记录在 `runtime_listener` 和 warnings 中，但不再单独等同于 deployment authority 变化；pre、post 和 recovery 使用同一语义。旧 8080 没有 `viewer_mode` 只表示 legacy schema；前后都缺失不会被误判为不可达。
