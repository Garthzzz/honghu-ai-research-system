...
def build_index(
    ...
    *,
    ...
    total_pages = 0
    total_characters = 0
    # Papers may be grouped into auditable subfolders such as
    # ``company_filings``.  Recursive discovery keeps one corpus index without
    # forcing users to flatten official filings and sell-side reports together.
    for path in sorted(
        papers_dir.rglob("*.pdf"),
        key=lambda item: item.relative_to(papers_dir).as_posix().lower(),
    ):
        ...
        try:
            if (
                ...
                character_count = int(
                    old.get("character_count")
                    or len(output.read_text(encoding="utf-8"))
                )
                cache_status = "reused"
            else:
    ...
    result = {
        "schema_version": "research.pdf_corpus.v1",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="为A/B轨研究资料夹建立逐页文本、文件哈希和抽取审计索引"
    )
