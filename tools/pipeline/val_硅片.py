#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""半导体硅片 #18 估值回填(yfinance,PE/PB/PS+市值)。亏损 PE=None 不造假。可溯源 dp。"""
...
conn = db_writer.get_db(); IND = 18; TODAY = date.today().isoformat()

# yfinance 源(复用或新建)
r = conn.execute("SELECT id FROM source WHERE fetch_method='api_yfinance'").fetchone()
if r: YF = r["id"]
else:
    YF = conn.execute("""INSERT INTO source(title,source_type,publisher,publish_date,quality_tier,value_layer,
        fetch_method,source_credibility,language,source_subtype) VALUES(?,?,?,?,?,?,?,?,?,?)""",
        ("yfinance 估值/财务 API","三方数据","Yahoo Finance",TODAY,3,"最新数据","api_yfinance","unverified","en","api")).lastrowid
print("yfinance source #", YF)

CUR = {".SS":("亿元","CNY"),".SZ":("亿元","CNY"),".T":("亿日元","JPY"),".TWO":("亿新台币","TWD"),".DE":("亿欧元","EUR")}
COS = [(510,"688126.SS"),(532,"605358.SS"),(544,"688432.SS"),(549,"688233.SS"),(547,"002129.SZ"),
       (507,"4063.T"),(543,"3436.T"),(509,"6488.TWO"),(539,"WAF.DE"),(545,"688584.SS"),(546,"003026.SZ")]
...
ok=0; fail=[]
for cid, tk in COS:
    sfx = "."+tk.split(".")[-1]; unit,cur = CUR.get(sfx,("亿","?"))
    try:
        ...
        conn.execute("""UPDATE company SET pe_ttm=?,pb=?,ps_ttm=?,market_cap_value=?,market_cap_unit=?,
            valuation_as_of=?,valuation_source_id=? WHERE id=?""",(pe,pb,ps,mc_yi,unit,TODAY,YF,cid))
...
conn.commit()
print(f"估值入库公司 {ok}/{len(COS)};失败/无数据: {fail}")
conn.close()
