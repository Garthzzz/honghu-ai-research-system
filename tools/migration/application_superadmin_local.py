...
def gui(runtime: Path, subject: str) -> int:
    ...
    tk.Label(frame, text="只修改研究平台应用账号，不授予 Windows、VM 或数据库系统权限。", fg="#666", wraplength=440, justify="left").pack(anchor="w", pady=(6, 18))
    tk.Label(frame, text=f"账号：{subject}").pack(anchor="w")
    first = tk.StringVar(); second = tk.StringVar(); reason = tk.StringVar(); visible = tk.BooleanVar(value=False)
    tk.Label(frame, text="新密码").pack(anchor="w", pady=(12, 3)); first_entry=tk.Entry(frame,textvariable=first,show="•"); first_entry.pack(fill="x")
    tk.Label(frame, text="确认密码").pack(anchor="w", pady=(10, 3)); second_entry=tk.Entry(frame,textvariable=second,show="•"); second_entry.pack(fill="x")
    def toggle():
        show="" if visible.get() else "•"; first_entry.configure(show=show); second_entry.configure(show=show)
    tk.Checkbutton(frame,text="显示密码",variable=visible,command=toggle).pack(anchor="w")
    tk.Label(frame, text="重置原因").pack(anchor="w", pady=(8, 3)); tk.Entry(frame,textvariable=reason).pack(fill="x")
    def submit():
        try:
            reset(runtime, subject, first.get(), second.get(), reason.get())
        except Exception as exc:
            first.set(""); second.set(""); messagebox.showerror("未完成", str(exc)); return
        first.set(""); second.set(""); messagebox.showinfo("已完成", "密码已重置，全部旧会话已撤销。")
        root.destroy()
    tk.Button(frame,text="确认重置",command=submit,bg="#9f3029",fg="white",font=("Microsoft YaHei UI",10,"bold"),padx=18,pady=7).pack(anchor="e",pady=(18,0))
    first_entry.focus_set(); root.mainloop(); return 0
