...
def finalize(runtime_path: Path, security_config_path: Path) -> None:
    ...
    with factory() as connection:
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT application_identity.local_set_authentication_proof_v1(%s)",
                (proof_sha,),
            )
            row = cursor.fetchone()
...
def main() -> int:
    ...
    finalize(args.runtime.resolve(), args.security_config.resolve())
    return 0
