...
`config/migration/target_rpo_rto_proposal.json` 把不可补抓的人工作品/发布 ledger、财务修订、动态任务状态、sentiment raw/aggregate 和 papers/evidence 分开提出目标。它当前是 `pending_human_approval`：未获人工批准前，禁止 production data cutover；阶段 5 的真实 restore 才能产生 measured RPO/RTO，不能把目标写成已达成事实。
